...
#ifdef USE_IIC3
LOCAL void riic3_eei3_hdr(UINT dintno)
{
UINT flgptn;
	flgptn = RIIC3.ICSR2.BYTE;
	RIIC3.ICSR2.BYTE &= 0xE7;				// Clear NACKF,STOP fo ICSR2
	IIC_Handler( IIC3, flgptn & IIC_EEI_INT );		// Call IIC Handler
}

LOCAL void riic3_rxi3_hdr(UINT dintno)
{
	IIC_Handler( IIC3, IIC_RXI_INT );			// Call IIC Handler
}

LOCAL void riic3_txi3_hdr(UINT dintno)
{
	IIC_Handler( IIC3, IIC_TXI_INT );			// Call IIC Handler
}

LOCAL void riic3_tei3_hdr(UINT dintno)
{
	RIIC3.ICSR2.BIT.TEND = 0;				// Clear TEND fo ICSR2
	IIC_Handler( IIC3, IIC_TEI_INT );			// Call IIC Handler
}
#endif /* USE_IIC3 */
...
union { T_CTSK t_ctsk; T_CFLG t_cflg; T_DDEV t_ddev; T_DINT t_dint; } u;
	...
	RIIC0.ICBRH.BIT.BRH = PCLK/4.F/1000000*IIC0_HPSCL-1E-6F;// SCL0 High Pulse Width Minimum
	RIIC0.ICBRL.BIT.BRL = PCLK/4.F/1000000*IIC0_LPSCL-1E-6F;// SCL0 Low  Pulse Width Minimum
...
//	RIIC0.ICMR3.BYTE = 0x11;				// ACKBT Write Enable, Noise Filter is 2
	...
	RIIC1.ICBRH.BIT.BRH = PCLK/4.F/1000000*IIC1_HPSCL-1E-6F;// SCL1 High Pulse Width Minimum
	RIIC1.ICBRL.BIT.BRL = PCLK/4.F/1000000*IIC1_LPSCL-1E-6F;// SCL1 Low  Pulse Width Minimum
...
//	RIIC1.ICMR3.BYTE = 0x11;				// ACKBT Write Enable, Noise Filter is 2
...
#ifdef CLANGSPEC
	u.t_dint.inthdr = riic1_tei1_hdr;			// Set Handler Address
#else
	u.t_dint.inthdr = (FP)riic1_tei1_hdr;			// Set Handler Address
#endif
	tk_def_int( VECT( RIIC1, TEI1 ), &u.t_dint );		// Define Interrupt Handler
#ifdef CLANGSPEC
	u.t_dint.inthdr = riic1_eei1_hdr;			// Set Handler Address
#else
	u.t_dint.inthdr = (FP)riic1_eei1_hdr;			// Set Handler Address
#endif
	tk_def_int( VECT( RIIC1, EEI1 ), &u.t_dint );		// Define Interrupt Handler
	...
	EnableInt( VECT( RIIC1, TEI1 ), IIC_CFG_INT_PRIORITY );	// Enable RIIC1 TEI1 Interrupt
	EnableInt( VECT( RIIC1, EEI1 ), IIC_CFG_INT_PRIORITY );	// Enable RIIC1 EEI1 Interrupt
	...
	RIIC2.ICBRH.BIT.BRH = PCLK/4.F/1000000*IIC2_HPSCL-1E-6F;// SCL2 High Pulse Width Minimum
	RIIC2.ICBRL.BIT.BRL = PCLK/4.F/1000000*IIC2_LPSCL-1E-6F;// SCL2 Low  Pulse Width Minimum
...
//	RIIC2.ICMR3.BYTE = 0x11;				// ACKBT Write Enable, Noise Filter is 2
...
#ifdef CLANGSPEC
	u.t_dint.inthdr = riic2_tei2_hdr;			// Set Handler Address
#else
	u.t_dint.inthdr = (FP)riic2_tei2_hdr;			// Set Handler Address
#endif
	tk_def_int( VECT( RIIC2, TEI2 ), &u.t_dint );		// Define Interrupt Handler
#ifdef CLANGSPEC
	u.t_dint.inthdr = riic2_eei2_hdr;			// Set Handler Address
#else
	u.t_dint.inthdr = (FP)riic2_eei2_hdr;			// Set Handler Address
#endif
	tk_def_int( VECT( RIIC2, EEI2 ), &u.t_dint );		// Define Interrupt Handler
	...
	EnableInt( VECT( RIIC2, TEI2 ), IIC_CFG_INT_PRIORITY );	// Enable RIIC2 TEI2 Interrupt
	EnableInt( VECT( RIIC2, EEI2 ), IIC_CFG_INT_PRIORITY );	// Enable RIIC2 EEI2 Interrupt
...
#ifdef USE_IIC3
	tk_dis_dsp( );						// Dispatch Disable
	if( ! MSTP( RIIC3 ) )  {				// RIIC3 is Already Enable ?
		tk_ena_dsp( );					// Dispatch Enable
		return E_OK;					// RIIC3 is Already Enable
	}
	SYSTEM.PRCR.WORD = 0xA502;				// Protect Disable
	MSTP( RIIC3 ) = 0;					// Enable RIIC3
	SYSTEM.PRCR.WORD = 0xA500;				// Protect Enable
	tk_ena_dsp( );						// Dispatch Enable
	RIIC3.ICCR1.BIT.IICRST = 1;				// RIIC3 Reset
	RIIC3.ICCR1.BIT.ICE = 1;				// SCL3,SDA3 Pins in Active State
	RIIC3.ICSER.BYTE = 0x00;				// SARL3,SARU3,General ID is Ignore
	RIIC3.ICMR1.BIT.CKS = 2;				// Base Clock = PCLKB/4(60/4=15MHz)
	RIIC3.ICBRH.BIT.BRH = PCLK/4.F/1000000*IIC3_HPSCL-1E-6F;// SCL3 High Pulse Width Minimum
	RIIC3.ICBRL.BIT.BRL = PCLK/4.F/1000000*IIC3_LPSCL-1E-6F;// SCL3 Low  Pulse Width Minimum
	RIIC3.ICMR2.BYTE = 0x00;				// TMOL,TMOH is Ignore
//	RIIC3.ICMR2.BYTE = 0x10;				// TMOL,TMOH is Ignore, SDA Delay Counter is 1
	RIIC3.ICMR3.BYTE = 0x10;				// ACKBT Write Enable, Noise Filter is 1
//	RIIC3.ICMR3.BYTE = 0x11;				// ACKBT Write Enable, Noise Filter is 2
	RIIC3.ICFER.BYTE = 0x50;				// SCLE,NACKE Enable
	RIIC3.ICIER.BYTE = 0x18;				// NAKIE,SPIE Enable
	RIIC3.ICCR1.BIT.IICRST = 0;				// Release from Internal Reset State
	tk_dis_dsp( );						// Dispatch Disable
	MPC.PWPR.BIT.B0WI = 0;					// PFSWE Write Enable
	MPC.PWPR.BIT.PFSWE = 1;					// PmnPFS Write Enable
	MPC.PC0PFS.BYTE = 0x0F;					// PC0 is SCL3 Pin
	MPC.PC1PFS.BYTE = 0x0F;					// PC1 is SDA3 Pin
	MPC.PWPR.BYTE = 0x80;					// Write Disable
	PORTC.ODR0.BYTE |= 0x05;				// SCL3,SDA3 is Open Drain
	PORTC.PMR.BYTE |= 0x03;					// PC0,PC1 is Peripheral Pin
	tk_ena_dsp( );						// Dispatch Enable
	u.t_dint.intatr = TA_HLNG;				// Set Handler Attribute
#ifdef CLANGSPEC
	u.t_dint.inthdr = riic3_rxi3_hdr;			// Set Handler Address
#else
	u.t_dint.inthdr = (FP)riic3_rxi3_hdr;			// Set Handler Address
#endif
	tk_def_int( VECT( RIIC3, RXI3 ), &u.t_dint );		// Define Interrupt Handler
#ifdef CLANGSPEC
	u.t_dint.inthdr = riic3_txi3_hdr;			// Set Handler Address
#else
	u.t_dint.inthdr = (FP)riic3_txi3_hdr;			// Set Handler Address
#endif
	tk_def_int( VECT( RIIC3, TXI3 ), &u.t_dint );		// Define Interrupt Handler
#ifdef CLANGSPEC
	u.t_dint.inthdr = riic3_tei3_hdr;			// Set Handler Address
#else
	u.t_dint.inthdr = (FP)riic3_tei3_hdr;			// Set Handler Address
#endif
	tk_def_int( VECT( RIIC3, TEI3 ), &u.t_dint );		// Define Interrupt Handler
#ifdef CLANGSPEC
	u.t_dint.inthdr = riic3_eei3_hdr;			// Set Handler Address
#else
	u.t_dint.inthdr = (FP)riic3_eei3_hdr;			// Set Handler Address
#endif
	tk_def_int( VECT( RIIC3, EEI3 ), &u.t_dint );		// Define Interrupt Handler
	EnableInt( VECT( RIIC3, RXI3 ), IIC_CFG_INT_PRIORITY );	// Enable RIIC3 RXI3 Interrupt
	EnableInt( VECT( RIIC3, TXI3 ), IIC_CFG_INT_PRIORITY );	// Enable RIIC3 TXI3 Interrupt
	EnableInt( VECT( RIIC3, TEI3 ), IIC_CFG_INT_PRIORITY );	// Enable RIIC3 TEI3 Interrupt
	EnableInt( VECT( RIIC3, EEI3 ), IIC_CFG_INT_PRIORITY );	// Enable RIIC3 EEI3 Interrupt
	iic_tbl[IIC3].drvname = "iicd";				// Set Driver Name
	iic_tbl[IIC3].iic = (void*)&RIIC3;			// Set RIIC Channel Address
	iic_tbl[IIC3].next = IIC_MAXIMUM;			// Set Next Command Pointer
	if( ( ercd = iicCreFlg( &iic_tbl[IIC3], &u.t_cflg ) ) < E_OK )
		goto ERROR;					// Create IIC EventFlag
	if( ( ercd = iicCreTsk( &iic_tbl[IIC3], IIC3, &u.t_ctsk ) ) < E_OK )
		goto ERROR;					// Create IIC Task
	if( ( ercd = iicDriverEntry( iic_tbl[IIC3].drvname, IIC3, &u.t_ddev ) ) < E_OK )
		goto ERROR;					// Define Device Driver
#endif /* USE_IIC3 */
	return E_OK;
ERROR:
