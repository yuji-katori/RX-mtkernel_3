...
union { T_CTSK t_ctsk; T_CFLG t_cflg; T_DDEV t_ddev; T_DINT t_dint; } u;
...
#elif defined(USE_SSCL4_PK4)
	MPC.PK4PFS.BYTE = 0x0A;					// PK4 is SSCL4 Pin
#endif
#if defined(USE_SSDA4_PB1)
	MPC.PB1PFS.BYTE = 0x0A;					// PB1 is SSDA4 Pin
#elif defined(USE_SSDA4_PK5)
	MPC.PK5PFS.BYTE = 0x0A;					// PK5 is SSDA4 Pin
#endif
	MPC.PWPR.BYTE = 0x80;					// Write Disable
	tk_ena_dsp( );						// Dispatch Enable
#if defined(USE_SSCL4_PB0)
...
#elif defined(USE_SSCL4_PK4)
	PORTK.ODR1.BIT.B0 = 1;					// SSCL4 is Open Drain
	PORTK.PMR.BIT.B4 = 1;					// PK4 is Peripheral Pin
#endif
#if defined(USE_SSDA4_PB1)
	PORTB.ODR0.BIT.B2 = 1;					// SSDA4 is Open Drain
	PORTB.PMR.BIT.B1 = 1;					// PB1 is Peripheral Pin
#elif defined(USE_SSDA4_PK5)
	PORTK.ODR1.BIT.B2 = 1;					// SSDA4 is Open Drain
	PORTK.PMR.BIT.B5 = 1;					// PK5 is Peripheral Pin
#endif
	u.t_dint.intatr = TA_HLNG;				// Set Handler Attribute
#ifdef CLANGSPEC
	...
	SCI7.BRR = PCLKB / 32.F / SIIC7_FSCL - 1;		// SCL Frequency
	SCI7.MDDR = (SCI7.BRR+1) * 8192.F * SIIC7_FSCL / PCLKB;	// Analize Modulation Duty
	...
	SCI9.BRR = PCLKB / 32.F / SIIC9_FSCL - 1;		// SCL Frequency
	SCI9.MDDR = (SCI9.BRR+1) * 8192.F * SIIC9_FSCL / PCLKB;	// Analize Modulation Duty
...
#elif defined(USE_SSCL9_PK3)
	MPC.PK3PFS.BYTE = 0x0A;					// PK3 is SSCL9 Pin
#endif
#if defined(USE_SSDA9_PB7)
	MPC.PB7PFS.BYTE = 0x0A;					// PB7 is SSDA9 Pin
#elif defined(USE_SSDA9_PK2)
	MPC.PK2PFS.BYTE = 0x0A;					// PK2 is SSDA9 Pin
#endif
	MPC.PWPR.BYTE = 0x80;					// Write Disable
	tk_ena_dsp( );						// Dispatch Enable
#if defined(USE_SSCL9_PB6)
...
#elif defined(USE_SSCL9_PK3)
	PORTK.ODR0.BIT.B6 = 1;					// SSCL9 is Open Drain
	PORTK.PMR.BIT.B3 = 1;					// PK3 is Peripheral Pin
#endif
#if defined(USE_SSDA9_PB7)
	PORTB.ODR1.BIT.B6 = 1;					// SSDA9 is Open Drain
	PORTB.PMR.BIT.B7 = 1;					// PB7 is Peripheral Pin
#elif defined(USE_SSDA9_PK2)
	PORTK.ODR0.BIT.B4 = 1;					// SSDA9 is Open Drain
	...
	MPC.PWPR.BIT.B0WI = 0;					// PFSWE Write Enable
	MPC.PWPR.BIT.PFSWE = 1;					// PmnPFS Write Enable
#if defined(USE_SSCL12_PA2)
	MPC.PA2PFS.BYTE = 0x0C;					// PA2 is SSCL12 Pin
#elif defined(USE_SSCL12_PE2)
	MPC.PE2PFS.BYTE = 0x0C;					// PE2 is SSCL12 Pin
#endif
#if defined(USE_SSDA12_PA4)
	MPC.PA4PFS.BYTE = 0x0C;					// PA4 is SSDA12 Pin
#elif defined(USE_SSDA12_PE1)
	MPC.PE1PFS.BYTE = 0x0C;					// PE1 is SSDA12 Pin
#endif
	MPC.PWPR.BYTE = 0x80;					// Write Disable
	tk_ena_dsp( );						// Dispatch Enable
#if defined(USE_SSCL12_PA2)
	PORTA.ODR0.BIT.B4 = 1;					// SSCL12 is Open Drain
	PORTA.PMR.BIT.B2 = 1;					// PA2 is Peripheral Pin
#elif defined(USE_SSCL12_PE2)
...
#endif
#if defined(USE_SSDA12_PA4)
	PORTA.ODR1.BIT.B0 = 1;					// SSDA12 is Open Drain
	PORTA.PMR.BIT.B4 = 1;					// PA4 is Peripheral Pin
#elif defined(USE_SSDA12_PE1)
