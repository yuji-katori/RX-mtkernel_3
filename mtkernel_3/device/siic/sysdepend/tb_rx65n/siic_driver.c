...
union { T_CTSK t_ctsk; T_CFLG t_cflg; T_DDEV t_ddev; T_DINT t_dint; } u;
	...
	tk_ena_dsp( );						// Dispatch Enable
	if( IPR( ICU, GROUPBL1 ) != SIIC_CFG_INT_PRIORITY )	// BL1 Group IPR Over IIC IPR ?
		return E_IO;
	...
	MPC.P86PFS.BYTE = 0x0A;					// P86 is SSCL10 Pin
#elif defined(USE_SSCL10_PC6)
	MPC.PC6PFS.BYTE = 0x24;					// PC6 is SSCL10 Pin
#endif
#if defined(USE_SSDA10_P82)
	MPC.P82PFS.BYTE = 0x0A;					// P82 is SSDA10 Pin
#elif defined(USE_SSDA10_P87)
	MPC.P87PFS.BYTE = 0x0A;					// P87 is SSDA10 Pin
#elif defined(USE_SSDA10_PC7)
	MPC.PC7PFS.BYTE = 0x24;					// PC7 is SSDA10 Pin
#endif
	MPC.PWPR.BYTE = 0x80;					// Write Disable
	tk_ena_dsp( );						// Dispatch Enable
#if defined(USE_SSCL10_P81)
	...
	tk_ena_dsp( );						// Dispatch Enable
	if( IPR( ICU, GROUPAL0 ) != SIIC_CFG_INT_PRIORITY )	// AL0 Group IPR Over IIC IPR ?
		return E_IO;
