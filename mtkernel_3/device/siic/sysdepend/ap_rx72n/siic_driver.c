...
union { T_CTSK t_ctsk; T_CFLG t_cflg; T_DDEV t_ddev; T_DINT t_dint; } u;
	...
	SCI0.BRR = PCLKB / 32.F / SIIC0_FSCL - 1;		// SCL Frequency
	SCI0.MDDR = (SCI0.BRR+1) * 8192.F * SIIC0_FSCL / PCLKB;	// Analize Modulation Duty
	...
	SCI2.BRR = PCLKB / 32.F / SIIC2_FSCL - 1;		// SCL Frequency
	SCI2.MDDR = (SCI2.BRR+1) * 8192.F * SIIC2_FSCL / PCLKB;	// Analize Modulation Duty
	if( SCI2.MDDR < 0x80 )					// Check  Modulation Duty
		return E_IO;
	...
	SCI3.BRR = PCLKB / 32.F / SIIC3_FSCL - 1;		// SCL Frequency
	SCI3.MDDR = (SCI3.BRR+1) * 8192.F * SIIC3_FSCL / PCLKB;	// Analize Modulation Duty
	if( SCI3.MDDR < 0x80 )					// Check  Modulation Duty
		return E_IO;
	...
	else if( IPR( ICU, GROUPBL0 ) != SIIC_CFG_INT_PRIORITY )// BL0 Group IPR Over IIC IPR ?
		return E_IO;
	...
	SCI4.BRR = PCLKB / 32.F / SIIC4_FSCL - 1;		// SCL Frequency
	SCI4.MDDR = (SCI4.BRR+1) * 8192.F * SIIC4_FSCL / PCLKB;	// Analize Modulation Duty
	if( SCI4.MDDR < 0x80 )					// Check  Modulation Duty
		return E_IO;
	...
	SCI6.BRR = PCLKB / 32.F / SIIC6_FSCL - 1;		// SCL Frequency
	SCI6.MDDR = (SCI6.BRR+1) * 8192.F * SIIC6_FSCL / PCLKB;	// Analize Modulation Duty
	...
	SCI7.BRR = PCLKA / 32.F / SIIC7_FSCL - 1;		// SCL Frequency
	SCI7.MDDR = (SCI7.BRR+1) * 8192.F * SIIC7_FSCL / PCLKA;	// Analize Modulation Duty
	if( SCI7.MDDR < 0x80 )					// Check  Modulation Duty
		return E_IO;
...
#elif defined(USE_SSCL7_PH1)
	MPC.PH1PFS.BYTE = 0x0A;					// PH1 is SSDL7 Pin
#endif
...
#elif defined(USE_SSDA7_PH2)
	MPC.PH2PFS.BYTE = 0x0A;					// PH2 is SSDA7 Pin
#endif
	MPC.PWPR.BYTE = 0x80;					// Write Disable
	tk_ena_dsp( );						// Dispatch Enable
#if defined(USE_SSCL7_P57)
	PORT5.ODR1.BIT.B6 = 1;					// SSCL7 is Open Drain
	PORT5.PMR.BIT.B7 = 1;					// P57 is Peripheral Pin
#elif defined(USE_SSCL7_P92)
	PORT9.ODR0.BIT.B4 = 1;					// SSCL7 is Open Drain
	PORT9.PMR.BIT.B2 = 1;					// P92 is Peripheral Pin
#elif defined(USE_SSCL7_PH1)
	PORTH.ODR0.BIT.B2 = 1;					// SSCL7 is Open Drain
...
#elif defined(USE_SSDA7_PH2)
	PORTH.ODR0.BIT.B4 = 1;					// SSDA7 is Open Drain
	...
	if( ! IPR( ICU, GROUPAL0 ) )  {				// AL0 Group IPR is Zero ?
		...
		EnableInt( VECT( ICU, GROUPAL0 ), SIIC_CFG_INT_PRIORITY );
	}							// Enable AL0 Interrupt
	GroupAL0Table[22] = sci7_tei7_hdr;			// Set Handler Address
	EN( SCI7, TEI7 ) = 1;					// Enable SCI7 TEI7 Group Interrupt
	tk_ena_dsp( );						// Dispatch Enable
	if( IPR( ICU, GROUPAL0 ) != SIIC_CFG_INT_PRIORITY )	// AL0 Group IPR != IIC IPR ?
		return E_IO;
	...
	SCI8.BRR = PCLKA / 32.F / SIIC8_FSCL - 1;		// SCL Frequency
	SCI8.MDDR = (SCI8.BRR+1) * 8192.F * SIIC8_FSCL / PCLKA;	// Analize Modulation Duty
...
#elif defined(USE_SSCL8_PK1)
	MPC.PK1PFS.BYTE = 0x0A;					// PK1 is SSDL8 Pin
#endif
#if defined(USE_SSDA8_PC7)
	MPC.PC7PFS.BYTE = 0x0A;					// PC7 is SSDA8 Pin
#elif defined(USE_SSDA8_PJ2)
	MPC.PJ2PFS.BYTE = 0x0A;					// PJ2 is SSDA8 Pin
#elif defined(USE_SSDA8_PK2)
	MPC.PK2PFS.BYTE = 0x0A;					// PK2 is SSDA8 Pin
#endif
	MPC.PWPR.BYTE = 0x80;					// Write Disable
	tk_ena_dsp( );						// Dispatch Enable
#if defined(USE_SSCL8_PC6)
...
#elif defined(USE_SSCL8_PJ1)
	PORTJ.ODR0.BIT.B2 = 1;					// SSCL8 is Open Drain
	PORTJ.PMR.BIT.B1 = 1;					// PJ1 is Peripheral Pin
#elif defined(USE_SSCL8_PK1)
	PORTK.ODR0.BIT.B2 = 1;					// SSCL8 is Open Drain
...
#elif defined(USE_SSDA8_PJ2)
	PORTJ.ODR0.BIT.B4 = 1;					// SSDA8 is Open Drain
	PORTJ.PMR.BIT.B2 = 1;					// PJ2 is Peripheral Pin
#elif defined(USE_SSDA8_PK2)
	PORTK.ODR0.BIT.B4 = 1;					// SSDA8 is Open Drain
	...
	GroupAL0Table[0] = sci8_tei8_hdr;			// Set Handler Address
	...
	SCI9.BRR = PCLKA / 32.F / SIIC9_FSCL - 1;		// SCL Frequency
	SCI9.MDDR = (SCI9.BRR+1) * 8192.F * SIIC9_FSCL / PCLKA;	// Analize Modulation Duty
...
#elif defined(USE_SSCL9_PL1)
	MPC.PL1PFS.BYTE = 0x0A;					// PL1 is SSCL9 Pin
#endif
#if defined(USE_SSDA9_PB7)
	MPC.PB7PFS.BYTE = 0x0A;					// PB7 is SSDA9 Pin
#elif defined(USE_SSDA9_PL2)
	MPC.PL2PFS.BYTE = 0x0A;					// PL2 is SSDA9 Pin
#endif
	MPC.PWPR.BYTE = 0x80;					// Write Disable
	tk_ena_dsp( );						// Dispatch Enable
#if defined(USE_SSCL9_PB6)
...
#elif defined(USE_SSCL9_PL1)
	PORTL.ODR0.BIT.B2 = 1;					// SSCL9 is Open Drain
...
#elif defined(USE_SSDA9_PL2)
	PORTL.ODR0.BIT.B4 = 1;					// SSDA9 is Open Drain
	...
	GroupAL0Table[4] = sci9_tei9_hdr;			// Set Handler Address
	...
	SCI10.BRR = PCLKA / 32.F / SIIC10_FSCL - 1;		// SCL Frequency
	SCI10.MDDR = (SCI10.BRR+1)*8192.F*SIIC10_FSCL/PCLKA;	// Analize Modulation Duty
	if( SCI10.MDDR < 0x80 )					// Check  Modulation Duty
		return E_IO;
...
#elif defined(USE_SSCL10_P86)
	MPC.P86PFS.BYTE = 0x24;					// P86 is SSCL10 Pin
#elif defined(USE_SSCL10_PC6)
	MPC.PC6PFS.BYTE = 0x0A;					// PC6 is SSCL10 Pin
#elif defined(USE_SSCL10_PM1)
	MPC.PM1PFS.BYTE = 0x0A;					// PM1 is SSCL10 Pin
#endif
#if defined(USE_SSDA10_P82)
	MPC.P82PFS.BYTE = 0x0A;					// P82 is SSDA10 Pin
#elif defined(USE_SSDA10_P87)
	MPC.P87PFS.BYTE = 0x24;					// P87 is SSDA10 Pin
#elif defined(USE_SSDA10_PC7)
	MPC.PC7PFS.BYTE = 0x0A;					// PC7 is SSDA10 Pin
#elif defined(USE_SSDA10_PM2)
	MPC.PM2PFS.BYTE = 0x0A;					// PM2 is SSDA10 Pin
#endif
	MPC.PWPR.BYTE = 0x80;					// Write Disable
	tk_ena_dsp( );						// Dispatch Enable
#if defined(USE_SSCL10_P81)
...
#elif defined(USE_SSCL10_P86)
	PORT8.ODR1.BIT.B4 = 1;					// SSCL10 is Open Drain
	PORT8.PMR.BIT.B6 = 1;					// P86 is Peripheral Pin
#elif defined(USE_SSCL10_PC6)
	PORTC.ODR1.BIT.B4 = 1;					// SSCL10 is Open Drain
	PORTC.PMR.BIT.B6 = 1;					// PC6 is Peripheral Pin
#elif defined(USE_SSCL10_PM1)
	PORTM.ODR0.BIT.B2 = 1;					// SSCL10 is Open Drain
...
#elif defined(USE_SSDA10_P87)
	PORT8.ODR1.BIT.B6 = 1;					// SSDA10 is Open Drain
	PORT8.PMR.BIT.B7 = 1;					// P87 is Peripheral Pin
#elif defined(USE_SSDA10_PC7)
	PORTC.ODR1.BIT.B6 = 1;					// SSCL10 is Open Drain
	PORTC.PMR.BIT.B7 = 1;					// PC7 is Peripheral Pin
#elif defined(USE_SSDA10_PM2)
	PORTM.ODR0.BIT.B4 = 1;					// SSCL10 is Open Drain
	...
	SCI11.BRR = PCLKA / 32.F / SIIC11_FSCL - 1;		// SCL Frequency
	SCI11.MDDR = (SCI11.BRR+1)*8192.F*SIIC11_FSCL/PCLKA;	// Analize Modulation Duty
	if( SCI11.MDDR < 0x80 )					// Check  Modulation Duty
		return E_IO;
...
#elif defined(USE_SSCL11_PB6)
	MPC.PB6PFS.BYTE = 0x24;					// PB6 is SSCL11 Pin
#elif defined(USE_SSCL11_PQ1)
	MPC.PQ1PFS.BYTE = 0x0A;					// PQ1 is SSCL11 Pin
#endif
#if defined(USE_SSDA11_P77)
	MPC.P77PFS.BYTE = 0x0A;					// P77 is SSDA11 Pin
#elif defined(USE_SSDA11_PB7)
	MPC.PB7PFS.BYTE = 0x24;					// PB7 is SSDA11 Pin
#elif defined(USE_SSDA11_PQ2)
	MPC.PQ2PFS.BYTE = 0x0A;					// PQ2 is SSDA11 Pin
#endif
	MPC.PWPR.BYTE = 0x80;					// Write Disable
	tk_ena_dsp( );						// Dispatch Enable
#if defined(USE_SSCL11_P76)
	...
	PORTB.PMR.BIT.B6 = 1;					// PB6 is Peripheral Pin
#elif defined(USE_SSCL11_PQ1)
	PORTQ.ODR0.BIT.B2 = 1;					// SSCL11 is Open Drain
	...
	PORTB.PMR.BIT.B7 = 1;					// PB7 is Peripheral Pin
#elif defined(USE_SSDA11_PQ2)
	PORTQ.ODR0.BIT.B4 = 1;					// SSDA11 is Open Drain
