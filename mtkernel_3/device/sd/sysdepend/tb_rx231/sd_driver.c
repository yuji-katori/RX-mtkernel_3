...
ER ercd;
	...
	if( SDHI.SDSTS2.LONG & 0x4000 )  {			// Command Sequence End ?
		while( !SDHI.SDSTS2.LONG & 0x2000 )  ;		// Wait SD Bus non Busy
		SDHI.SDCLKCR.LONG = 0x0120;			// PCLKB/128
		while( SDHI.SDSTS2.LONG & 0x4000 )  ;		// Wait Command Sequence End
		while( !SDHI.SDSTS2.LONG & 0x2000 )  ;		// Wait SD Bus non Busy
		SDHI.SDCLKCR.LONG = 0x0320;			// PCLKB/128, Auto Clock
	}
...
UINT res;
	...
	while( !SDHI.SDSTS2.LONG & 0x2000 )  ;			// Wait SD Bus non Busy
	SDHI.SDCLKCR.LONG = 0x0120;				// PCLKB(27MHz)/128Å‡210.937KHz
//	Must be 74Cycle Provide					// 74Cycle/210.937KHz=350.8156ns
	for( i=0 ; i<4736  ; i++ )  ;				// 350.8156ns/54MHz*4cyc=4736
	while( !SDHI.SDSTS2.LONG & 0x2000 )  ;			// Wait SD Bus non Busy
	SDHI.SDCLKCR.LONG = 0x0320;				// PCLKB/128, Auto Clock
...
UINT r1, r2, devsize, dsize_mult;
	...
	if( ercd < E_OK || SDHI.SDRSP10 & OCR_ERRORBITS )	// Error Check
		goto ERROR;
	SDCard.Protect = ( SDHI.SDSTS1.LONG & 0x0080 ) != 0;	// Set Write Protect
	return E_OK;
ERROR:
...
EXPORT ER SDC_Init(ID objid, T_DINT *p_dint)
{
	...
	MPC.PB1PFS.BYTE = 0x1A;					// Set SDHI_CLK Pin
	MPC.PB0PFS.BYTE = 0x1A;					// Set SDHI_CMD Pin
	MPC.PB5PFS.BYTE = 0x1A;					// Set SDHI_CD Pin
	MPC.PC3PFS.BYTE = 0x1A;					// Set SDHI_D0 Pin
	MPC.PC4PFS.BYTE = 0x1A;					// Set SDHI_D1 Pin
	MPC.PB7PFS.BYTE = 0x1A;					// Set SDHI_D2 Pin
	MPC.PC2PFS.BYTE = 0x1A;					// Set SDHI_D3 Pin
	MPC.PWPR.BYTE = 0x80;					// Write Disable

	PORTB.PCR.BYTE |= 0xA1;					// Pull Up Mos ON
	PORTB.PMR.BYTE |= 0xA3;					// Enable Peripheral Pin
	PORTC.PCR.BYTE |= 0x1C;					// Pull Up Mos ON
	PORTC.PMR.BYTE |= 0x1C;					// Enable Peripheral Pin
	tk_ena_dsp( );						// Dispatch Enable
...
#ifdef CLANGSPEC
	p_dint->inthdr = SD_Detect_hdr;				// Set Handler Address
#else
	p_dint->inthdr = (FP)SD_Detect_hdr;			// Set Handler Address
#endif
	tk_def_int( VECT( SDHI, CDETI ), p_dint );		// Define Interrupt Handler
#ifdef CLANGSPEC
	p_dint->inthdr = SD_Int_hdr;				// Set Handler Address
#else
	p_dint->inthdr = (FP)SD_Int_hdr;			// Set Handler Address
#endif
	tk_def_int( VECT( SDHI, CACI ), p_dint );		// Define Interrupt Handler

	EnableInt( VECT(SDHI, CDETI), SD_CFG_INT_PRIORITY );	// Enable BL1 Group Interrupt
	EnableInt( VECT(SDHI, CACI ), SD_CFG_INT_PRIORITY );	// Enable BL1 Group Interrupt
