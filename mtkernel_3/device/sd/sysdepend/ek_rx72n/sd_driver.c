...
EXPORT ER SDC_Init(ID objid, T_DINT *p_dint)
{
	...
	PORTG.PDR.BIT.B0 = 1;					// PG0 Pin is Output(Select SDC)
	MPC.PWPR.BIT.B0WI = 0;					// PFSWE Write Enable.
	MPC.PWPR.BIT.PFSWE = 1;					// PmnPFS Write Enable.
	MPC.P21PFS.BYTE = 0x1A;					// Set SDHI_CLK Pin
	MPC.P20PFS.BYTE = 0x1A;					// Set SDHI_CMD Pin
	MPC.P25PFS.BYTE = 0x1A;					// Set SDHI_CD Pin
	MPC.P22PFS.BYTE = 0x1A;					// Set SDHI_D0 Pin
	MPC.P23PFS.BYTE = 0x1A;					// Set SDHI_D1 Pin
	MPC.P87PFS.BYTE = 0x1A;					// Set SDHI_D2 Pin
	MPC.P17PFS.BYTE = 0x1A;					// Set SDHI_D3 Pin
	MPC.PWPR.BYTE = 0x80;					// Write Disable

	PORT2.PMR.BYTE |= 0x2F;					// P20-P23,P25 Enable Peripheral Pin
	PORT8.PMR.BIT.B7 = 1;					// P87 Enable Peripheral Pin
	PORT1.PMR.BIT.B7 = 1;					// P17 Enable Peripheral Pin
	tk_ena_dsp( );						// Dispatch Enable
...
EXPORT PRI SDC_GetTaskPri(void)
{
