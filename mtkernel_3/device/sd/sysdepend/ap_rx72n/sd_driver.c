 ...
 *----------------------------------------------------------------------
 *    Modified by Yuji Katori at 2023/09/08.
 *    Modified by Yuji Katori at 2023/09/11.
...
EXPORT ER SDC_Init(ID objid, T_DINT *p_dint)
{
	...
	MPC.PM0PFS.BYTE = 0x1A;					// Set SDHI_CLK Pin
	MPC.PM1PFS.BYTE = 0x1A;					// Set SDHI_CMD Pin
	MPC.PM6PFS.BYTE = 0x1A;					// Set SDHI_CD Pin
	MPC.PM2PFS.BYTE = 0x1A;					// Set SDHI_D0 Pin
	MPC.PM3PFS.BYTE = 0x1A;					// Set SDHI_D1 Pin
	MPC.PM4PFS.BYTE = 0x1A;					// Set SDHI_D2 Pin
	MPC.PM5PFS.BYTE = 0x1A;					// Set SDHI_D3 Pin
	MPC.PWPR.BYTE = 0x80;					// Write Disable

	PORTM.PMR.BYTE |= 0x7F;					// Enable Peripheral Pin
	tk_ena_dsp( );						// Dispatch Enable
