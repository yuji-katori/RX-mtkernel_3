...
typedef	enum {
	DN_DISKEVENT	= TDN_EVENT,	/* Message buffer for event notification	data: ID		RW */
	DN_DISKINFO	= TDN_DISKINFO,	/* Disk information				data: DiskInfo		R- */
	DN_DISKFORMAT	= -100,		/* Disk format					data: DiskFormat	-W */
	DN_DISKINIT	= -101,		/* Disk initialization				data: DiskInit		-W */
	DN_DISKCMD	= -102,		/* Disk command					data: DiskCmd		-W */
	DN_DISKMEMADR	= -103,		/* First address of memory disk area		data: void*		R- */
	DN_DISKPARTINFO	= -104,		/* Disk partition information			data: DiskPartInfo	R- */
	DN_DISKCHSINFO	= -105,		/* Disk CHS information				data: DiskCHSInfo	R- */
	DN_DISKIDINFO	= -106		/* Disk identifying information			data: UB[]		R- */
} DiskDataNo;
