...
{
	SCI_RXI_Handler( RSCIA );				// Call SCI_RXI_Handler
}

LOCAL void sci10_txi10_hdr(UINT dintno)
{
	SCI_TXI_Handler( RSCIA );				// Call SCI_TXI_Handler
}

LOCAL void sci10_tei10_hdr(UINT dintno)
{
	SCI10.SCR.BIT.TEIE = 0;					// Disable TEIE Interrupt
	SCI_TEI_Handler( RSCIA );				// Call SCI_TEI_Handler
}
#endif /* USE_SCI10 */
...
{
	SCI_RXI_Handler( RSCIB );				// Call SCI_RXI_Handler
}

LOCAL void sci11_txi11_hdr(UINT dintno)
{
	SCI_TXI_Handler( RSCIB );				// Call SCI_TXI_Handler
}

LOCAL void sci11_tei11_hdr(UINT dintno)
{
	SCI11.SCR.BIT.TEIE = 0;					// Disable TEIE Interrupt
	SCI_TEI_Handler( RSCIB );				// Call SCI_TEI_Handler
}
#endif /* USE_SCI11 */
...
union { T_CTSK t_ctsk; T_CFLG t_cflg; T_DDEV t_ddev; T_DINT t_dint; } u;
	...
	if( IPR( ICU, GROUPAL0 ) != SCI_CFG_INT_PRIORITY )	// AL0 Group IPR != SCI IPR ?
		return E_IO;
	sci_tbl[RSCIA].drvname = "scik";			// Set Driver Name
	sci_tbl[RSCIA].sci = (void*)&SCI10;			// Set SCI Channel Address
	sci_tbl[RSCIA].next = SCI_MAXIMUM;			// Set Next Command Pointer
	sci_tbl[RSCIA].pclk = PCLKA;				// Set Peripheral Clock
	if( ( ercd = sciCreFlg( &sci_tbl[RSCIA], &u.t_cflg ) ) < E_OK )
		goto ERROR;					// Create SCI EventFlag
	if( ( ercd = sciCreTsk( &sci_tbl[RSCIA], RSCIA, &u.t_ctsk ) ) < E_OK )
		goto ERROR;					// Create SCI Task
	if( ( ercd = sciDriverEntry( sci_tbl[RSCIA].drvname, RSCIA, &u.t_ddev ) ) < E_OK )
		goto ERROR;					// Define Device Driver
#endif /* USE_SCI10 */
	...
	if( IPR( ICU, GROUPAL0 ) != SCI_CFG_INT_PRIORITY )	// AL0 Group IPR != SCI IPR ?
		return E_IO;
	sci_tbl[RSCIB].drvname = "scil";			// Set Driver Name
	sci_tbl[RSCIB].sci = (void*)&SCI11;			// Set SCI Channel Address
	sci_tbl[RSCIB].next = SCI_MAXIMUM;			// Set Next Command Pointer
	sci_tbl[RSCIB].pclk = PCLKA;				// Set Peripheral Clock
	if( ( ercd = sciCreFlg( &sci_tbl[RSCIB], &u.t_cflg ) ) < E_OK )
		goto ERROR;					// Create SCI EventFlag
	if( ( ercd = sciCreTsk( &sci_tbl[RSCIB], RSCIB, &u.t_ctsk ) ) < E_OK )
		goto ERROR;					// Create SCI Task
	if( ( ercd = sciDriverEntry( sci_tbl[RSCIB].drvname, RSCIB, &u.t_ddev ) ) < E_OK )
		goto ERROR;					// Define Device Driver
#endif /* USE_SCI11 */
	...
	MPC.PWPR.BIT.B0WI = 0;					// PFSWE Write Enable
	MPC.PWPR.BIT.PFSWE = 1;					// PmnPFS Write Enable
#if defined(USE_RXD12_PA2)
	MPC.PA2PFS.BYTE = 0x0C;					// PA2 is RXD12 Pin
#elif defined(USE_RXD12_PE2)
	MPC.PE2PFS.BYTE = 0x0C;					// PE2 is RXD12 Pin
#endif
#if defined(USE_TXD12_PA4)
	MPC.PA4PFS.BYTE = 0x0C;					// PA4 is TXD12 Pin
#elif defined(USE_TXD12_PE1)
