...
#ifdef CLANGSPEC
typedef ER			(*OPNFP)(ID,UINT,void *);		/* Open function address general */
typedef ER			(*CLSFP)(ID,UINT,void *);		/* Close function address general */
typedef ER			(*EXCFP)(T_DEVREQ *,TMO,void *);	/* Execute function address general */
typedef INT			(*WAIFP)(T_DEVREQ *,INT,TMO,void *);	/* Completion wait function address general */
typedef ER			(*ABTFP)(ID,T_DEVREQ *,INT,void *) ;	/* Abort function address general */
typedef INT			(*EVTFP)(INT,void *,void *);		/* Event function address general */
#endif /* CLANGSPEC */
...
typedef struct t_ddev {
	...
	SZ	blksz;		/* Specific data block size (-1: Unknown) */
#ifdef CLANGSPEC
	OPNFP	openfn;		/* Open function */
	CLSFP	closefn;	/* Close function */
	EXCFP	execfn;		/* Execute function */
	WAIFP	waitfn;		/* Completion wait function */
	ABTFP	abortfn;	/* Abort function */
	EVTFP	eventfn;	/* Event function */
#else
